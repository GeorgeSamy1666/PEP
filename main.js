...
let arr = [];
let particleCount1 = isMobile ? 50 : 110;
let maxSize1 = isMobile ? 1.2 : 2.5;
...
class P {
    constructor() { 
        ...
        this.s = 0.7 + Math.random() * maxSize1; 
    }
    update() { 
    ...
    draw() { 
        q.beginPath(); 
        q.arc(this.x, this.y, this.s, 0, Math.PI * 2); 
        q.fillStyle = 'rgba(100,210,255,0.95)'; 
        q.shadowBlur = 12; q.shadowColor = 'rgba(100,210,255,0.8)'; 
        q.fill(); q.shadowBlur = 0; 
    }
}
for (let i = 0; i < particleCount1; i++) arr.push(new P());
...
function resizeNet() { canvasNet.width = innerWidth; canvasNet.height = innerHeight; }
resizeNet(); addEventListener('resize', resizeNet);

let nodes = [];
let nodeCount = isMobile ? 60 : 120;
...
class Node {
  ...
  draw() {
    ...
    ctxNet.shadowBlur = 12; ctxNet.shadowColor = 'rgba(120,220,255,0.8)';
    ctxNet.fill(); ctxNet.shadowBlur = 0;
  }
}
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      ...
      if (dist < 150) {
        ...
        ctxNet.strokeStyle = `rgba(120,220,255,${1 - dist / 110})`;
...
function animateNet() { ctxNet.clearRect(0, 0, canvasNet.width, canvasNet.height); nodes.forEach(n => { n.update(); n.draw(); }); connectNodes(); requestAnimationFrame(animateNet); }
animateNet();
...
function resizeNet3() { canvasNet3.width = innerWidth; canvasNet3.height = innerHeight; }
resizeNet3(); addEventListener('resize', resizeNet3);

let nodes3 = [];
let nodeCount3 = isMobile ? 50 : 100;
let maxSize3 = isMobile ? 1.0 : 2.0;
...
class Node3 {
  constructor() {
    ...
    this.r = 0.7 + Math.random() * maxSize3;
  }
  update() {
    this.x += this.vx; this.y += this.vy;
  ...
  draw() {
    ...
    ctxNet3.fillStyle = 'rgba(160,230,255,0.8)';
    ctxNet3.shadowBlur = 10; ctxNet3.shadowColor = 'rgba(160,230,255,0.8)';
    ctxNet3.fill(); ctxNet3.shadowBlur = 0;
  }
}
for (let i = 0; i < nodeCount3; i++) nodes3.push(new Node3());

function connectNodes3() {
  for (let i = 0; i < nodes3.length; i++) {
    for (let j = i + 1; j < nodes3.length; j++) {
      let dx = nodes3[i].x - nodes3[j].x, dy = nodes3[i].y - nodes3[j].y, dist = Math.hypot(dx, dy);
      if (dist < 130) {
        ctxNet3.beginPath();
        ctxNet3.moveTo(nodes3[i].x, nodes3[i].y);
        ctxNet3.lineTo(nodes3[j].x, nodes3[j].y);
        ctxNet3.strokeStyle = `rgba(160,230,255,${1 - dist / 110})`;
        ctxNet3.lineWidth = 0.5;
        ctxNet3.stroke();
      }
    }
  }
}
function animateNet3() { ctxNet3.clearRect(0, 0, canvasNet3.width, canvasNet3.height); nodes3.forEach(n => { n.update(); n.draw(); }); connectNodes3(); requestAnimationFrame(animateNet3); }
animateNet3();
...
function resizeCanvas4() { canvas4.width = innerWidth; canvas4.height = innerHeight; }
resizeCanvas4(); addEventListener('resize', resizeCanvas4);

let dots4 = [];
let dotCount4 = isMobile ? 40 : 80;
let maxSize4 = isMobile ? 1.0 : 2.0;
...
class Dot4 {
  constructor() {
    ...
    this.vx = (Math.random() - 0.5) * 0.5;
    this.vy = (Math.random() - 0.5) * 0.5;
    this.radius = 0.7 + Math.random() * maxSize4;
  }
  update() {
    this.x += this.vx; this.y += this.vy;
  ...
  draw() {
    ...
    ctx4.fillStyle = 'rgba(200,240,255,0.8)';
    ctx4.shadowBlur = 7; ctx4.shadowColor = 'rgba(200,240,255,0.8)';
    ctx4.fill(); ctx4.shadowBlur = 0;
  }
}
for (let i = 0; i < dotCount4; i++) dots4.push(new Dot4());

function linkDots4() {
  for (let i = 0; i < dots4.length; i++) {
    for (let j = i + 1; j < dots4.length; j++) {
      let dx = dots4[i].x - dots4[j].x, dy = dots4[i].y - dots4[j].y, dist = Math.hypot(dx, dy);
      if (dist < 120) {
        ctx4.beginPath();
        ctx4.moveTo(dots4[i].x, dots4[i].y);
        ctx4.lineTo(dots4[j].x, dots4[j].y);
        ctx4.strokeStyle = `rgba(200,240,255,${1 - dist / 110})`;
        ctx4.lineWidth = 0.5;
        ctx4.stroke();
      }
    }
...
function animate4() { ctx4.clearRect(0, 0, canvas4.width, canvas4.height); dots4.forEach(d => { d.update(); d.draw(); }); linkDots4(); requestAnimationFrame(animate4); }
animate4();
